...
def load_sample_data():
    ...
    try:
        sample_scripts = [
            {
                ...
                'path': 'C:\\Scripts\\Oracle\\DDL\\create_users_table.sql',
                'category': 'BD',
                'priority': 'high',
                'status': 'applied',
                'responsible': 'DBA Team',
                'notes': 'Tabla principal de usuarios del sistema',
                'date_applied': datetime(2024, 1, 15).isoformat()
            },
            {
                'name': 'alter_products_add_column.sql',
                'path': 'C:\\Scripts\\Oracle\\Alters\\alter_products_add_column.sql',
                'category': 'Alters',
                'priority': 'medium',
                'status': 'pending',
                'responsible': 'DBA Team',
                'notes': 'Agregar columna de descuento a tabla productos'
            },
            {
                'name': 'update_customer_data.sql',
                'path': 'C:\\Scripts\\Oracle\\Updates\\update_customer_data.sql',
                'category': 'Updates',
                'priority': 'high',
                'status': 'error',
                'responsible': 'DBA Team',
                'notes': 'Error: Constraint violation en campo email'
            },
            {
                'name': 'view_sales_report.sql',
                'path': 'C:\\Scripts\\Oracle\\Views\\view_sales_report.sql',
                'category': 'Vistas',
                'priority': 'low',
                'status': 'applied',
                'responsible': 'DBA Team',
                'notes': 'Vista para reportes de ventas mensuales',
                'date_applied': datetime(2024, 1, 10).isoformat()
            },
            {
                'name': 'grant_permissions.sql',
                'path': 'C:\\Scripts\\Oracle\\Permisos\\grant_permissions.sql',
                'category': 'Permisos',
                'priority': 'high',
                'status': 'pending',
                'responsible': 'DBA Team',
                'notes': 'Otorgar permisos de lectura a usuario APP_USER'
            },
            {
                'name': 'script_migration_001.sql',
                'path': 'C:\\Scripts\\Oracle\\Migration\\script_migration_001.sql',
                'category': 'Scripts ID',
                'priority': 'medium',
                'status': 'applied',
                'responsible': 'DBA Team',
                'notes': 'Migración inicial de datos legacy',
                'date_applied': datetime(2024, 1, 8).isoformat()
            }
        ]
        ...
        return jsonify({
            'success': True,
            'message': f'{imported_count} scripts de ejemplo cargados',
            'imported_count': imported_count
        })
